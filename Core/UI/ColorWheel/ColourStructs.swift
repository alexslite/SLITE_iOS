...
struct RGB {
    ...
    var color: Color {
        return Color(cube256: .sRGB, red: Int(r * 255.0), green: Int(g * 255.0), blue: Int(b * 255.0))
    }
    ...
    init(hexadecimal: String, alpha: CGFloat = 1.0) {
        ...
        self.init(r: CGFloat((rgbValue & 0xFF0000) >> 16) / 255.0, g: CGFloat((rgbValue & 0x00FF00) >> 8) / 255.0, b: CGFloat(rgbValue & 0x0000FF) / 255.0)
    }
    
    var hexValue: UInt32 {
        var value : UInt32 = 0
        value += UInt32(r * 255.0) << 16 |
                           UInt32(g * 255.0) << 8 |
                           UInt32(b * 255.0) << 0
        return value
    }
...
struct HSV {
    ...
    var fullBrightness: HSV {
        return HSV(h: self.h, s: self.s, v: 1)
    }
}
